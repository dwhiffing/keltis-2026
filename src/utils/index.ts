...
import { NUM_DISCARD_PILES, NUM_SUITS } from './constants'
...
export const getCardPilePosition = (
  ...
  const pileType =
    card.pileIndex === 0
      ? 'deck'
      : card.pileIndex === 1 ||
          card.pileIndex === NUM_SUITS * 2 + NUM_DISCARD_PILES + 2
        ? 'hand'
        : card.pileIndex > 1 + NUM_SUITS &&
  ...
  if (pileType === 'tableau') {
    ...
    const isOwnTableau =
      localPlayerIndex === 0
        ...
        : card.pileIndex < 2 + NUM_SUITS + NUM_DISCARD_PILES
    offsetY =
      card.cardPileIndex * (CARD_Y_GAP * width) * (isOwnTableau ? 1 : -1)
  }
  if (pileType === 'hand') {
    const { width } = getPileSize()
